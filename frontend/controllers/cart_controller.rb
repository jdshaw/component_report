...
class CartController < ApplicationController
  ...
  def download_report
    ...
    tmp_file = Tempfile.new("component_report_#{SecureRandom.hex}.xlsx")

    report_response = JSONModel::HTTP.post_form("/plugins/component_report/repositories/#{session[:repo_id]}/report", "uri[]" => uris)
    tmp_file.write(report_response.body)

    send_file tmp_file, :type => "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", :disposition => "attachment", :filename => "component_report.xlsx"
    ...
    #       #response.headers['Content-Disposition'] = "inline; filename=\"component_report.xlsx\""
    ...
    #self.response_body = QueueStreamer2.new(queue)
    ...
    #       end
    #
    #       #send_me = [chunk.bytesize.to_s(16), "\r\n", chunk, "\r\n"].join
    #       #send_me = [chunk.bytesize.to_s(16), "\r\n", chunk, "\r\n"].join
    #       #p "-- SEND ME: #{send_me}"
    #
    #       block.call(chunk)
